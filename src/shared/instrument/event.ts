...
export type NextEvent = EventWithSource &
  TypedEvent<'next'> & { value: unknown };
export type ErrorEvent = EventWithSource &
  TypedEvent<'error'> & { error: unknown };
...
export function serialize(event: Event): {} {
  return match<{}>({
    ...
    error: (errorEvent) => ({
      ...errorEvent,
      error: JSON.stringify(errorEvent.error),
    }),
    next: (nextEvent) => ({
      ...nextEvent,
      value: JSON.stringify(nextEvent.value),
    }),
    subscribe: identity,
...
export function sourceFromStackFrame(
